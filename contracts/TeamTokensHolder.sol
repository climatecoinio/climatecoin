...
contract TeamTokensHolder is Owned {
    ...
    ClimateContribution crowdsale;
    MiniMeToken climate;

    function TeamTokensHolder(address _owner, address _crowdsale, address _climate) {
        owner = _owner;
        crowdsale = ClimateContribution(_crowdsale);
    ...
    function collectTokens() public onlyOwner {
        uint256 balance = climate.balanceOf(address(this));
        ...
        assert(climate.transfer(owner, canExtract));
    ...
    function claimTokens(address _token) public onlyOwner {
        require(_token != address(climate));
        if (_token == 0x0) {
